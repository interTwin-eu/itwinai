...
from itwinai.models.tensorflow.cyclones_vgg import custom_VGG_V1, custom_VGG_V2, custom_VGG_V3, ModelV5
...
def get_network_config(network, **kwargs):
    ...
    if network == Network.VGG_V1.value:
        ...
        model = custom_VGG_V1(patch_size=kwargs['patch_size'], channels=kwargs['channels'],
                              activation=kwargs['activation'], regularizer=kwargs['regularizer'])

    elif network == Network.VGG_V2.value:
        print('Using custom VGG V2')
        model = custom_VGG_V2(patch_size=kwargs['patch_size'], channels=kwargs['channels'],
                              activation=kwargs['activation'], regularizer=kwargs['regularizer'])

    elif network == Network.VGG_V3.value:
        print('Using custom VGG V3')
        model = custom_VGG_V3(patch_size=kwargs['patch_size'], channels=kwargs['channels'],
                              activation=kwargs['activation'], regularizer=kwargs['regularizer'])

    elif network == Network.MODEL_V5.value:
        print('Using Model V5')
        model = ModelV5(patch_size=kwargs['patch_size'], channels=kwargs['channels'],
                        last_activation=kwargs['activation'], kernel_size=kwargs['kernel_size'])
