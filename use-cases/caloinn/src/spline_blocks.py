...
class CubicSplineBlock(fm.InvertibleModule):
    ...
    def __init__(
        ...
        self.bounds = self.bounds_activation(
            torch.ones(1, self.splits[1], *([1] * self.input_rank)) * float(bounds)
        )
        self.tails = tails
        ...
        self.w_perm = nn.Parameter(
            w.view(channels, channels, *([1] * self.input_rank)), requires_grad=False
        )
        self.w_perm_inv = nn.Parameter(
            w.T.view(channels, channels, *([1] * self.input_rank)), requires_grad=False
        )
        ...
        self.subnet = subnet_constructor(
            ...
            (2 * self.num_bins + 2) * self.splits[1],
        )
        self.last_jac = None
    ...
    def _unconstrained_cubic_spline(self, inputs, theta, rev=False):
        ...
        derivatives_left = min_bin_width + torch.sigmoid(unnorm_derivatives_left) * (
            3 * slopes[..., [0]] - 2 * min_bin_width
        )
        derivatives_right = min_bin_width + torch.sigmoid(unnorm_derivatives_right) * (
            3 * slopes[..., [-1]] - 2 * min_bin_width
        )
        ...
        if rev:
            ...
            mask_index = torch.argsort(1.0 * masks, dim=-1, descending=True)[..., [0]]
    ...
    def _permute(self, x, rev=False):
        ...
        if rev:
            return (self.permute_function(x * scale, self.w_perm_inv), perm_log_jac)
        else:
    ...
    def forward(self, x, c=[], rev=False, jac=True):
        ...
        if not rev:
            theta = self.subnet(x1c).reshape(
                x1c.shape[0], self.splits[1], 2 * self.num_bins + 2
            )
...
class RationalQuadraticSplineBlock(fm.InvertibleModule):
    ...
    def __init__(
        ...
        self.w_perm = nn.Parameter(
            torch.Tensor(w).view(channels, channels, *([1] * self.input_rank)),
            requires_grad=False,
        )
        self.w_perm_inv = nn.Parameter(
            torch.Tensor(w.T).view(channels, channels, *([1] * self.input_rank)),
            requires_grad=False,
        )
