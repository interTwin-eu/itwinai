...
class RNNDistributedTrainer(TorchTrainer):
    ...
    def init_hython_trainer(self) -> None:
        # self.config.loss_fn = instantiate(OmegaConf.create({"loss_fn": self.config.loss_fn}))[
        #     "loss_fn"
        # ]
        ...
        if self.config.hython_trainer == "rnntrainer":
            self.model = self.model_class(
                ...
                head_kwargs=self.config.model_head_kwargs
                if self.config.model_head_kwargs is not None
                else {},
            )
            ...
            model_pt = (
                ...
                / self.config.model_head_dir
                / self.config.model_head_file
            )
...
class RNNDistributedRayTrainer(RayTorchTrainer):
    """Trainer class for RNN model using pytorch.
    ...
    def create_model_loss_optimizer(self) -> None:
        ...
        if isinstance(self.strategy, RayDeepSpeedStrategy):
            # Batch size definition is not optional for RayDeepSpeedStrategy!
            distribute_kwargs = {
                "config_params": {
                    "train_micro_batch_size_per_gpu": self.training_config.batch_size
                }
            }
        # elif isinstance(self.strategy, RayDDPStrategy):
        #     if "find_unused_parameters" not in self.config.model_fields:
        #         self.config.find_unused_parameters = False
        #     distribute_kwargs = {"find_unused_parameters": self.config.find_unused_parameters}
    ...
    def train(self, config, data):
        ...
        for epoch in tqdm(range(self.epochs)):
            ...
            self.checkpoint_and_report(
                epoch=epoch, tuning_metrics={"loss": avg_val_loss.item()}
            )
