...
class ConvRNNDatasetGetterAndSplitter(DataSplitter):
    ...
    def execute(
        ...
        # Dataset preparation
        Xd = (
            read_from_zarr(url=self.surrogate_input, group="xd")
            .sel(time=self.train_temporal_range)[self.dynamic_names]
        )
        Xs = read_from_zarr(
            url=self.surrogate_input, group="xs")[self.static_names]
        Y = (
            read_from_zarr(url=self.surrogate_input, group="y")
            .sel(time=self.train_temporal_range)[self.target_names]
        )

        Y_test = (
            read_from_zarr(url=self.surrogate_input, group="y")
            .sel(time=self.test_temporal_range)[self.target_names]
        )

        Xd_test = (
            read_from_zarr(url=self.surrogate_input , group="xd")
            .sel(time=self.test_temporal_range)[self.dynamic_names]
        )
        masks = (
            read_from_zarr(url=self.surrogate_input, group="mask")
            .mask.sel(mask_layer=self.mask_names)
