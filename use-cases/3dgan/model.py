...
class Generator(nn.Module):
    def __init__(self, latent_dim):  # img_shape
        ...
        self.up1 = nn.Upsample(scale_factor=(
            6, 6, 6), mode='trilinear', align_corners=False)
        self.conv1 = nn.Conv3d(in_channels=8, out_channels=8,
        ...
        self.conv2 = nn.Conv3d(in_channels=8, out_channels=6,
        ...
        self.conv3 = nn.Conv3d(in_channels=6, out_channels=6,
                               kernel_size=(4, 4, 6), padding=0)
        nn.init.kaiming_uniform_(self.conv3.weight)
        self.bn3 = nn.BatchNorm3d(num_features=6, eps=1e-6)
        self.pad3 = nn.ConstantPad3d((1, 1, 2, 2, 2, 2), 0)
        self.conv4 = nn.Conv3d(in_channels=6, out_channels=6,
                               kernel_size=(4, 4, 6), padding=0)
        ...
        self.conv5 = nn.Conv3d(in_channels=6, out_channels=6,
        ...
        self.conv6 = nn.Conv3d(in_channels=6, out_channels=6,
                               kernel_size=(3, 3, 3), padding=0)
        nn.init.kaiming_uniform_(self.conv6.weight)
        self.conv7 = nn.Conv3d(in_channels=6, out_channels=1,
...
class Discriminator(nn.Module):
    def __init__(self, power):
        ...
        self.conv3 = nn.Conv3d(in_channels=8, out_channels=8,
                               kernel_size=(5, 6, 6), padding=0)
        self.bn2 = nn.BatchNorm3d(num_features=8, eps=1e-6)
        self.drop3 = nn.Dropout(0.2)

        self.conv4 = nn.Conv3d(in_channels=8, out_channels=8,
                               kernel_size=(5, 6, 6), padding=0)
        ...
        # The input features for the Linear layer need to be calculated based on the output shape from the previous layers.
    ...
    def ecal_angle(self, image, daxis1):
        ...
        # Angle Calculations
        z = (torch.arange(z_shape, device=image.device, dtype=torch.float32) + 0.5) * \
            torch.ones_like(z_ref)  # Make an array of z indexes for all events
...
class ThreeDGAN(pl.LightningModule):
    ...
    def compute_global_loss(self, labels, predictions, loss_weights=[3, 0.1, 25, 0.1]):
        ...
        mean_example_loss_1 = mean_absolute_percentage_error_object1 * \
            loss_weights[1]
        ...
        mean_example_loss_2 = mean_absolute_percentage_error_object2 * \
            loss_weights[3]
    ...
    def training_step(self, batch, batch_idx):
        ...
        if fake_batch_loss[3] == 100.0 and self.index > 10:
            print(
                "Empty image with Ecal loss equal to 100.0 for {} batch".format(self.index))
    ...
    def validation_step(self, batch, batch_idx):
        ...
        self.log('val_discriminator_loss', sum(
            disc_eval_loss), on_epoch=True, prog_bar=True)
