...
class AtmoRepTrainer(TorchTrainer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        assert "res" in cf.path_data, Exception("Resolution not in file name. Please specify it.")
...
###################################################
    ...
    def execute(self) -> None:
        ...
        # parallelized there)
        self.pre_batch = functools.partial( prepare_batch_BERT_multifield, cf, self.rngs,
                                                            cf.fields, cf.BERT_strategy )
        #init wandb
  ...
  ###################################################s
    ...
    def create_model_loss_optimizer(self) -> None:
        ...
        if cf.with_ddp :
            self.model = torch.nn.parallel.DistributedDataParallel( self.model, static_graph=True)
