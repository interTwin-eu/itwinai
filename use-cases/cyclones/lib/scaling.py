...
def fit_transform(volume,shape,channel,feature_range=(0,1),type='minmax',save=False, filename=None):
    """
    ...
    filename : pathlike
        The filename to which the scaler must be saved to disk. Checked only if 'save' is set to True.
    ...
    volume = scaler.fit_transform(volume.reshape(-1,channel)).reshape(-1,*shape,channel)
...
def get_scalers(scaler_X_file=None,scaler_y_file=None):
    ...
    Input:
        @ (string) scaler_X_file : X data scaler of joblib. None if data must not be scaled
        @ (string) scaler_y_file : y data scaler of joblib. None if data must not be scaled
    Output:
...
def save_tf_minmax(Xt, outfile):
    ...
    def scaler_encoding_fn(min, max):
        """Builds a serialized version of the dataset. X and y must be np.array."""
...
def minmax_inverse_transform(scaled_data,scaler):
    """
    Applies the inverse transform of TFMinMaxScaler to the provided scaled dataset.
    """
