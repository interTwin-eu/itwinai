...
if past_evaluation:
    for season in ["winter_", "spring_", "summer_", "autumn_"]:
        ...
        pd.DataFrame(tot_train_losses).to_csv(
            f"../outputs/train_losses_{season}1d_allssp.csv"
        )
        pd.DataFrame(tot_test_losses).to_csv(
            f"../outputs/test_losses_{season}1d_allssp.csv"
        )
...
if future_evaluation:
    for season in ["winter_", "spring_", "summer_", "autumn_"]:
        ...
        for scenario in ["585", "370", "245", "126"]:
            ...
            proj_data = np.load(
                f"../input/preprocessed_1d_proj{scenario}_{season}data_allssp.npy"
            )
            ...
            pd.DataFrame(tot_proj_losses).to_csv(
                f"../outputs/proj{scenario}_losses_{season}1d_allssp.csv"
            )
            print(
