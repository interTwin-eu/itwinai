...
from .constants import SLURM_TEMPLATE, DEFAULT_SLURM_SAVE_DIR
...
class SlurmScriptConfiguration(BaseModel):
    ...
    #: Optional job name for the SLURM job.
    job_name: str | None = None
    #: Billing account to charge the job to.
    account: str = "intertwin"
    #: Partition/queue the job should run on.
    partition: str = "develbooster"
    #: Wall-clock time limit for the job (HH:MM:SS).
    time: str = "00:30:00"
    ...
    #: Memory per node requested.
    memory: str = "16G"
    #: Whether to request exclusive node access.
    exclusive: bool = False

    #: Pre-execution command content (shell).
    ...
    #: Main execution command content (shell).
    ...
    # Builder behavior
    #: Whether to save the generated SLURM script.
    save_script: bool = False
    #: Whether to submit the generated SLURM script.
    submit_job: bool = False
    #: Directory where the script should be saved.
    save_dir: Path | None = Field(
        default=Path(DEFAULT_SLURM_SAVE_DIR),
        description="Directory to write the generated SLURM script.",
    )
    #: Path/URL to a pre-exec file to load content from.
    pre_exec_file: str | None = None
    #: Path/URL to an exec file to load content from.
...
class MLSlurmBuilderConfig(SlurmScriptConfiguration):
    ...
    #: Whether to launch jobs via Ray.
    use_ray: bool = False
    #: Optional container path to export.
    container_path: Path | None = None
    #: Distributed strategy to use for training. This field is required to avoid silent
    #: discrepancies with what passed to the ``TorchTrainer`` class.
    distributed_strategy: Literal["ddp", "horovod", "deepspeed"]
    #: Execution mode: single job, all strategies, or scaling test.
    ...
    #: Python virtual environment to activate.
    python_venv: str | None = None
    #: Hydra config name to pass to exec-pipeline.
    config_name: str = "config"
    #: Hydra config path to pass to exec-pipeline.
    config_path: str = "."
    #: Pipeline key to execute.
    pipe_key: str = "training_pipeline"
    #: Node counts to use for scaling tests.
    ...
    py_spy: bool = False
    #: Sampling rate for py-spy profiling.
