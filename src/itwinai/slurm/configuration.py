...
class SlurmScriptConfiguration(BaseModel):
    ...
    #: Optional job name for the SLURM job. Default: None (auto-generated later).
    job_name: str | None = None
    #: Billing account to charge the job to. Default: "intertwin".
    account: str = "intertwin"
    #: Partition/queue the job should run on. Default: "develbooster".
    partition: str = "develbooster"
    #: Wall-clock time limit for the job (HH:MM:SS). Default: "00:30:00".
    time: str = "00:30:00"

    #: Path to standard output file. Default: None (filled later).
    std_out: Path | None = None
    #: Path to standard error file. Default: None (filled later).
    err_out: Path | None = None

    #: Number of nodes requested. Default: 1.
    num_nodes: int = 1
    #: Number of tasks per node. Default: 1.
    num_tasks_per_node: int = 1
    #: GPUs per node requested. Default: 4.
    gpus_per_node: int = 4
    #: CPUs per task requested. Default: 16.
    cpus_per_task: int = 16
    #: Memory per node requested. Default: "16G".
    memory: str = "16G"
    #: Whether to request exclusive node access. Default: False.
    exclusive: bool = False

    #: Pre-execution command content (shell). Default: None (set by builder).
    ...
    #: Main execution command content (shell). Default: None (set by builder).
    ...
    # Builder behavior
    #: Whether to save the generated SLURM script. Default: False.
    save_script: bool = False
    #: Whether to submit the generated SLURM script. Default: False.
    submit_job: bool = False
    #: Directory where the script should be saved. Default: {DEFAULT_SLURM_SAVE_DIR}.
    save_dir: Path | None = Field(
        default=Path(DEFAULT_SLURM_SAVE_DIR),
        description=("Directory to write the generated SLURM script. "
        f"Default: {DEFAULT_SLURM_SAVE_DIR}"),
    )
    #: Path/URL to a pre-exec file to load content from. Default: None.
    pre_exec_file: str | None = None
    #: Path/URL to an exec file to load content from. Default: None.
...
class MLSlurmBuilderConfig(SlurmScriptConfiguration):
    ...
    #: Whether to launch jobs via Ray. Default: False.
    use_ray: bool = False
    #: Optional container path to export. Default: None.
    container_path: Path | None = None
    #: Distributed strategy to use for training. Required.
    distributed_strategy: Literal["ddp", "horovod", "deepspeed"]
    #: Execution mode: single job, all strategies, or scaling test. Default: "single".
    ...
    #: Python virtual environment to activate. Default: None.
    python_venv: str | None = None
    #: Hydra config name to pass to exec-pipeline. Default: "config".
    config_name: str = "config"
    #: Hydra config path to pass to exec-pipeline. Default: ".".
    config_path: str = "."
    #: Pipeline key to execute. Default: "training_pipeline".
    pipe_key: str = "training_pipeline"
    #: Node counts to use for scaling tests. Default: [1, 2, 4, 8].
    ...
    #: Enable py-spy profiling. Default: False.
    py_spy: bool = False
    #: Sampling rate for py-spy profiling. Default: 10.
    profiling_sampling_rate: int = 10
    #: Run name for tracking. Default: "main-run".
